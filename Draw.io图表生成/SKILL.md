...
### 2.1 厂商图标库（🚀 优先使用）
项目内置了各大厂商的专业图标库，使用 `shape=mxgraph.前缀.图标名` 即可调用。
...
- **高度计算规则**（⚠️ 重要）：
  - 生成前必须计算容器高度：`height = max(子元素底部位置) + 10~20 保留边距`
  - 每行子元素计算：`y + height` 得到底部位置
  - 多行取最大值，确保内容不溢出
  - 容器高度调整后，需同步调整后续层级和箭头的 y 坐标
...
- **容器尺寸统一**（⚠️ 重要）：
  ...
  - 计算容器宽度公式：
    ```
    容器宽 = 左边距(40) + Σ(子元素宽) + (子元素数-1)×间距(10) + 右边距(40)
    ```
  - 子元素 x 坐标计算（从 40 开始，每个元素递增 `前元素宽+10`）
  - 容器 x 坐标居中：`(页面宽 - 容器宽) / 2`
  - 内部边距统一标准：
    - 左边距：`40` 单位（第一个子元素 x 坐标）
    - 上边距：`title y=10`，`子元素 y=50`
    - 右边距：`40` 单位（容器宽 - 最右子元素的 x+width）
    - 下边距：`40` 单位（容器高 - 最下子元素的 y+height）
  - 容器高度根据内容计算：`height = max(子元素底部) + 40(下边距)`
- **字体大小规范**：
  - 主标题：`fontSize=22`
  - 层标题：`fontSize=16`
  - 子元素内容：`fontSize=14`
...
| 用途 | 填充色 | 边框色 |
|------|--------|--------|
| 第一层/入口 | `#E8F5E9` / `#C8E6C9` | `#66BB6A` |
| 第二层/处理 | `#E3F2FD` / `#BBDEFB` | `#42A5F5` |
| 第三层/决策 | `#FFF3E0` / `#FFE0B2` | `#FF9800` |
| 第四层/输出 | `#F3E5F5` / `#E1BEE7` | `#9C27B0` |
| 背景容器 | 同色系浅色 | 同色系深色 |
